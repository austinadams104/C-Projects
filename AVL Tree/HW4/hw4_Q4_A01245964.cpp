...
void AvlTree::rightRotate (AvlNode * & v)
{
	...
	k2->left = k1->right;
	k1->right = k2;
	if (getHeight(k2->left) > getHeight(k2->right)){
		k2->height = 1 + getHeight(k2->left);
	}
	else{
		k2->height = 1 + getHeight(k2->right);
	}
	if (getHeight(k1->left) > getHeight(k1->right)){
		k1->height = 1 + getHeight(k1->left);
	}
	else{
		k1->height = 1 + getHeight(k1->right);
	}
	v = k1;
}
